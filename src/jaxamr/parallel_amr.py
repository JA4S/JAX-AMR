...
def get_refinement_grid_mask(level, blk_data, blk_info, criterion, dx, dy):
    ...
    mask_x = jnp.maximum(jnp.abs(grad_x / (dx*2.0)) - amr.refinement_tolerance[criterion], 0)
    mask_y = jnp.maximum(jnp.abs(grad_y / (dy*2.0)) - amr.refinement_tolerance[criterion], 0)

    mask = jnp.sign(mask_x + mask_y)


    def extension_mask(mask):
        extended_mask = jnp.sign(convolve2d(mask, amr.grid_mask_buffer_kernel, mode='same'))
...
@partial(pmap,axis_name='x',static_broadcasted_argnums=0,in_axes=(None,0))
...
def get_refinement_block_mask(level, ref_grid_mask):
    ...
    ref_grid_mask = ref_grid_mask.reshape(ref_grid_mask.shape[0],
                        amr.n_block[level][0], amr.n_grid[level][0],
                        amr.n_block[level][1], amr.n_grid[level][1]).transpose(0, 1, 3, 2, 4)
...
@partial(pmap,axis_name='x',static_broadcasted_argnums=2,in_axes=(blk_info_pmap_axis,0,0))
...
@partial(pmap,axis_name='x',static_broadcasted_argnums=0,in_axes=(None,0,blk_info_pmap_axis))
@partial(jit, static_argnames=('level'))
def get_refinement_block_data(level, blk_data, ref_blk_info):

    blk_data = blk_data.reshape(blk_data.shape[0], blk_data.shape[1],
                amr.n_block[level][0], amr.n_grid[level][0],
                amr.n_block[level][1], amr.n_grid[level][1]).transpose(0, 1, 2, 4, 3, 5)
...
def interpolate_fine_to_coarse(level, blk_data, ref_blk_data, ref_blk_info):
    ...
    updated_blk_data = (
                ...
                .set(blk_data[:, :, -amr.n_grid[level][0]:, -amr.n_grid[level][1]:])
                .transpose(0, 1, 2, 4, 3, 5)
                .reshape(updated_blk_data.shape[0], updated_blk_data.shape[1],
                    amr.n_block[level][0] * amr.n_grid[level][0],
                    amr.n_block[level][1] * amr.n_grid[level][1])
    )
...
@partial(pmap,axis_name='x',in_axes=(blk_info_pmap_axis, blk_info_pmap_axis))
@jit
...
@partial(pmap,axis_name='x',in_axes=(0,0))
